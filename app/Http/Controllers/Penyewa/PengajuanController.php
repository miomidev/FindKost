...
class PengajuanController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $cekSewaAktif = PengajuanSewa::where('user_id', Auth::id())
            ->whereIn('status', ['aktif', 'jatuh_tempo'])
            ->exists();

        if ($cekSewaAktif) {
            return back()->with('error', 'Anda masih memiliki sewa yang aktif. Selesaikan atau akhiri masa sewa terlebih dahulu sebelum mengajukan sewa baru.');
        }
        ...
        if ($kamar->status !== 'tersedia') {
            return back()->with('error', 'Kamar yang Anda pilih saat ini sedang tidak tersedia.');
        }
