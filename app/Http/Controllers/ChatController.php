...
class ChatController extends Controller
{
    ...
    public function indexPenyewa()
    {
        ...
        $chats = Chat::with(['pemilik', 'kos', 'latestMessage'])
            ...
            ->where(function($q) {
                $q->whereNull('penyewa_cleared_at')
                  ->orWhereHas('messages', function($q2) {
                      $q2->whereColumn('chat_messages.created_at', '>', 'chats.penyewa_cleared_at');
                  });
            })
            ->get()
    ...
    public function indexPemilik()
    {
        ...
        $chats = Chat::with(['penyewa', 'kos', 'latestMessage'])
            ...
            ->where(function($q) {
                $q->whereNull('pemilik_cleared_at')
                  ->orWhereHas('messages', function($q2) {
                      $q2->whereColumn('chat_messages.created_at', '>', 'chats.pemilik_cleared_at');
                  });
            })
            ->get()
            ->sortByDesc(function ($chat) {
    ...
    public function show($id)
    {
        ...
        $chat = Chat::with(['penyewa', 'pemilik', 'kos'])
        ...
        $clearedAt = Auth::user()->role === 'penyewa' ? $chat->penyewa_cleared_at : $chat->pemilik_cleared_at;
        
        $chat->setRelation('messages', $chat->messages()->with('sender')
    ...
    public function destroy($id)
    {
        ...
        // Hapus pesan terkait terlebih dahulu jika tidak ada onDelete cascade
        if (Auth::user()->role === 'penyewa') {
            $chat->update(['penyewa_cleared_at' => now()]);
        } else {
            $chat->update(['pemilik_cleared_at' => now()]);
        }

        if (request()->ajax() || request()->wantsJson()) {
            return response()->json(['success' => true, 'message' => 'Obrolan berhasil dihapus.']);
        }

        if (Auth::user()->role === 'penyewa') {
            return redirect()->route('penyewa.chat.index')->with('success', 'Obrolan berhasil dihapus.');
        }
        return redirect()->route('pemilik.chat.index')->with('success', 'Obrolan berhasil dihapus.');
    }
}
