                        ...
                        <div class="row g-4">
                            ...
                            @forelse($kos as $k)
                                @php
                                    ...
                                    // Filter kamar sesuai SEMUA filter yang aktif
                                    ...
                                    if (!empty($filterTipeHarga)) {
                                        $kamarFiltered = $kamarFiltered->where('tipe_harga', $filterTipeHarga);
                                    }
                                    if (!empty($filterFasilitas)) {
                                        $kamarFiltered = $kamarFiltered->filter(function ($kamar) use ($filterFasilitas) {
                                            $fas = is_array($kamar->fasilitas) ? $kamar->fasilitas : [];
                                            foreach ($filterFasilitas as $f) {
                                                if (!in_array($f, $fas)) return false;
                                            }
                                            return true;
                                        });
                                    }

                                    // Jika ada filter aktif, tampilkan hanya kamar yg match
                                    $hasFilter = !empty($filterMinHarga) || !empty($filterMaxHarga) || !empty($filterTipeHarga) || !empty($filterFasilitas);
                                    $kamarDisplay = $hasFilter ? $kamarFiltered : $kamarTersedia;
                                    $kamarTermurah = $kamarDisplay->sortBy('harga')->first();
                                    $kamarTermahal = $kamarDisplay->sortBy('harga')->last();
                                    $tipeHargaList = $kamarDisplay->pluck('tipe_harga')->unique();
                                @endphp
